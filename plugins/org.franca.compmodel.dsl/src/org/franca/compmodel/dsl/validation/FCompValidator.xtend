	...
	def selfContainment(FCPrototype proto) {
		...
		if (parentType == protoType) 
			error('Component \'' + parentType.name + '\' must not contain prototype of component, which is already present in hierarchy', proto, 
					FC_GENERIC_PROTOTYPE__COMPONENT)
		if(parentType.superType == protoType)
		error('Component \'' + parentType.name + '\' must not contain prototype of parent component \'' + parentType.superType.name + '\', which is already present in hierarchy', proto, 
					FC_GENERIC_PROTOTYPE__COMPONENT)
		while(parentType.superType !== null)
		{
		parentType = parentType.superType
		var parentTypeName = proto.eContainer as FCComponent
		if(parentType.superType == protoType)
		error('Component \'' + parentTypeName.name + '\' must not contain prototype of parent component \'' + parentType.superType.name + '\', which is already present in hierarchy', proto, 
					FC_GENERIC_PROTOTYPE__COMPONENT)
		}
