...
public class FCompDeployLinker extends LazyLinker {
	...
	override void beforeModelLinked(EObject model, IDiagnosticConsumer diagnosticsConsumer) {
		...
		while (instances.hasNext) {
			...
			val URI uri = URI.createURI(instance.name + FCompDeployLinkingService.DOMAIN)
			...
			if (resource != null) {
				...
				for(Resource r : toBeDeleted) {
					try {
						...
						println("FCompLinker ERROR deleting resource" + r)
						e.printStackTrace()
					}
				}
			}
