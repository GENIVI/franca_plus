...
class DeviceAccessorGenerator {
	...
	def generate(FDSpecification spec) {
		...
		'''
			...
			public static class DevicePropertyAccessor
				«IF spec.base!==null»extends «spec.base.qualifiedClassname».DevicePropertyAccessor«ENDIF»
				implements Enums
			{
				...
				«addNeededFrancaType("FDeployedDevice")»
	...
	def private genProperties(FDDeclaration decl, ICodeContext context) '''
		«IF decl.properties.size > 0 && decl.host.isDeviceHost»
	...
	def private genProperty(FDPropertyDecl it, FDPropertyHost host, ICodeContext context) {
		val francaType = host.francaTypeDevice
