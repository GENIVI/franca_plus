...
class ImportManager {
		...
		val single =
			if (typeRef.complex===null) {
				switch (typeRef.predefined) {
					...
					case FDPredefinedTypeId::CONSTANT: {
						neededFrancaTypes.add("FConstantDef")
						"FConstantDef"
					}
					case FDPredefinedTypeId::ENUMERATOR: {
						neededFrancaTypes.add("FEnumerator")
						"FEnumerator"
					}
				}
			} else {
	...
	def private getFullImportPath(String t) {
		if (t.equals("EObject"))
			...
			"org.franca.compdeploymodel.core." + t
		else if (t.isDeploymentType)
			"org.franca.compdeploymodel.dsl.fDeploy." + t
		else
