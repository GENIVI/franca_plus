...
class ServiceAccessorGenerator {
	...
	def generate(FDSpecification spec) {
		...
		'''
			...
			public static class ServicePropertyAccessor
				«IF spec.base!==null»extends «spec.base.qualifiedClassname».ServicePropertyAccessor«ENDIF»
				implements Enums
			{
				...
				«addNeededFrancaType("FDeployedService")»
	...
	def private genProperties(FDDeclaration decl, ICodeContext context) '''
		«IF decl.properties.size > 0 && decl.host.isServiceHost»
	...
	def private genProperty(FDPropertyDecl it, FDPropertyHost host, ICodeContext context) {
		val francaType = host.francaTypeService
